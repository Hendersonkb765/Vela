...
const MembersList = ({ members, invitations }) => {
    ...
    //${member.status == 'PENDENTE' &&  "border-warning text-warning"} ${member.status == 'EXPIRADO' &&  "border-neutral-500 text-neutral-500 dark:border-gray-400 dark:text-gray-400"}`}
                ...
                {filteredMembers.length > 0? (
                    ...
                    filteredMembers.map((member) => (
                        ...
                        <li key={member.id} className="py-2 flex items-center gap-2">
                            ...
                            <p className={`ml-auto mr-4 px-2 py-1 text-xs border rounded-full border-solid border-success text-success ${member.status.toUpperCase() == 'MEMBRO'} ${member.status.toUpperCase() == 'PENDENTE' &&  "border-warning text-warning"} ${member.status.toUpperCase() == 'EXPIRADO' &&  "border-neutral-500 text-neutral-500 dark:border-gray-400 dark:text-gray-400"}`}>{member.status.toUpperCase()}</p>
                        </li>
                    ))
