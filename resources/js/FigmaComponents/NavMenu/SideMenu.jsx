...
const SideMenu = ({}) => {
    ...
    return (
        <div
            ...
            className={` ${open ? 'w-60' : 'w-20'} h-screen flex flex-col py-4 border-r border-neutralcolors-100 duration-300 `}
        >

            <ApplicationLogo className='w-full mb-4'/>
            ...
            <ul className='flex flex-col h-full px-2 gap-y-2'>
                {Menus.map((menu, index) => (
                    <Link
                        key={index}
                        className={`
                            ${open ? 'space-x-2 pl-4' : 'justify-center'} w-full h-14 flex items-center  text-base cursor-pointer hover:bg-neutralcolors rounded-md duration-150 '
                            ${index == 4 && 'mb-auto'}
                    ...
                    >
                        <div >{menu.icon}</div>
                        <span className={`${!open && 'hidden'} origin-left duration-500 text-body`}>{menu.title}</span>
                    </Link>
                ))}
            </ul>
        </div>
